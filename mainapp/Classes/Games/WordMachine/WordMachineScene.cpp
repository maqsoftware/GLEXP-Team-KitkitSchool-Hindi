                ...
                for (int i = worksheet.beginProblemID(); i < worksheet.endProblemID(); i++) {
                    ...
                    std::string resPath = "WordMachine/WordCards/";
                    {
                        ...
                        if (!FileUtils::getInstance()->isFileExist(sound)) {
                            if (find(_missings.begin(), _missings.end(), p.sound) ==
                                _missings.end()) {
                    ...
                    }
                    {
                        ...
                        if (!FileUtils::getInstance()->isFileExist(image)) {
                            if (find(_missings.begin(), _missings.end(), p.goodImage) ==
                                _missings.end()) {
                    ...
                    }
                    {
                        ...
                        if (!FileUtils::getInstance()->isFileExist(image)) {
                            if (find(_missings.begin(), _missings.end(), p.badImage) ==
                                _missings.end()) {
...
//    }
    ...
    _startButton->loadTextures("WordMachine/Images/wm_button_1.png",
                               "WordMachine/Images/wm_button_2.png");
    ...
    _progressBar->setPosition(
            Vec2(winSize.width / 2, winSize.height - _progressBar->getContentSize().height));
...
void WordMachineScene::initWheels(int numWheels, std::string types) {
    ...
    auto slotX = _gameNode->getContentSize().width / 2 -
                 slotWidth * (numWheels - 1) / 2.0;// _gameSize.width/2-815;
    ...
    if (_targetIsLegit) {
        ...
        auto abCallFunc = [this, alter, ab]() {
            ab->addClickEventListener([this, alter](Ref *) {
                ...
                alter->runAction(
                        Sequence::create(EaseOut::create(MoveBy::create(0.05, Vec2(-30, 0)), 2.0),
                ...
                StrictLogManager::shared()->game_Peek_Answer("WordMachine", workPath,
                                                             _currentProblem.badImage,
                                                             _currentProblem.goodImage);
            });
        };
        ...
        auto tbCallFunc = [this, target, alter, audioPath, tb, ab]() {
            tb->addClickEventListener([this, target, alter, audioPath, ab](Ref *btn) {
                ...
                target->setScale(1.1);
                GameSoundManager::getInstance()->playEffectSound(
                        "Common/Sounds/Effect/UI_Star_Collected.m4a");
                ...
                target->runAction(Sequence::create(DelayTime::create(0.5),
                                                   CallFunc::create([audioPath]() {
                                                       VoiceMoldManager::shared()->speak(
                                                               _speakMachineWord);  //Implementation of TTS for this module

                                                   }),
                                                   DelayTime::create(1.5),
                                                   EaseIn::create(MoveBy::create(0.3, Vec2(0,
                                                                                           panelSize.height)),
                                                                  2.0),
...
LetterWheel *WordMachineScene::CreateWheel(WordType wordType, bool playTick) {
    ...
    w->setup(wordType == WordType::Consonant ? consonantString : vowelString, Size(400, 700), 2.0,
             270, 0.6, playTick);
