...
BEGIN_NS_EGGQUIZ ;
    ...
    map<string, float> ProblemBank::loadDurationsheet() {
        ...
        std::string rawString = cocos2d::FileUtils::getInstance()->getStringFromFile(
                soundsPath + "durations.tsv");
    ...
    vector<EggQuizTemplateStruct>
    ProblemBank::loadProblemData(string course, string level, int *sheetNo) {
        srand((unsigned int) time(NULL));

        transform(level.begin(), level.end(), level.begin(), ::tolower);
        string rawString = FileUtils::getInstance()->getStringFromFile(
                tsvPath + "eggquiz" + course + "_levels_" +
        ...
        for (auto row : data) {
            ...
            if (row[0] != "") {
                ...
                types[rawType] = {rawType, templateName, questionOption1, questionOption2,
                                  questionOption3, questionOption4, questionOption5, answer,
                                  answerOption1};
            }
        }
        ...
        for (auto it : sequence) {
            ...
            if (type.templateName == "single_digit_numbers"
                ...
                while (1) {
                    ...
                    if (hasSameAnswer(type.templateName, type.questionOption1, fixedAnswer))
                        continue;
                ...
                random_shuffle(questionTypes.begin(), questionTypes.end(),
                               [](int n) { return rand() % n; });
                ...
                random_shuffle(questionTypes2.begin(), questionTypes2.end(),
                               [](int n) { return rand() % n; });
                ...
                while (1) {
                    int newChoice = random < int >
                                    (TodoUtil::stoi(answerVec[0]), TodoUtil::stoi(answerVec[1]));
                ...
                int position = (type.questionOption2 == "") ? random < int > (1, 4)
                                                            : TodoUtil::stoi(type.questionOption2);

                vector<string> ascOrDesc = {"ascending", "descending"};
                if (type.questionOption3 == "")
                    type.questionOption3 = ascOrDesc.at(random < int > (0, 1));
                ...
                while (1) {
                    ...
                    if (type.questionOption3 == "ascending") {
                        if (answerCandidate + step * 3 > max) continue;
                            ...
                            vector<int> answerVec = {answerCandidate, answerCandidate + step * 1,
                                                     answerCandidate + step * 2,
                                                     answerCandidate + step * 3};
                            if (multiple != 0 && answerVec.at(position - 1) % multiple != 0)
                                continue;
                            answerRange = getJoinString(answerVec);
                            if (hasSameAnswer(type.templateName, type.questionOption1,
                                              answerRange))
                                continue;
                            break;
                        }
                ...
                while (1) {
                    ...
                    if (result >= TodoUtil::stoi(answer[0]) &&
                        result <= TodoUtil::stoi(answer[1])) {
                        if (fomula.size() == 5) {
                            if (type.questionOption2 == "regroupingtrue" &&
                                !checkRegrouping(TodoUtil::stoi(fomula[0]), fomula[1][0], result))
                                continue;
                            if (type.questionOption2 == "regroupingfalse" &&
                                checkRegrouping(TodoUtil::stoi(fomula[0]), fomula[1][0], result))
                                continue;
                        }
                        ...
                        if (hasSameAnswer(type.templateName, questionOption1Candidate,
                                          answerRangeCandidate))
                            continue;
                ...
                while (1) {
                    int newChoice =
                            random < int > (TodoUtil::stoi(answer[0]), TodoUtil::stoi(answer[1]));
                    if (find(choices.begin(), choices.end(), newChoice) != choices.end()) continue;
                    choices.push_back(newChoice);

                    if (answerInt == 0) answerInt = newChoice;
                    if (type.questionOption1 == "biggest" && answerInt < newChoice)
                        answerInt = newChoice;
                    else if (type.questionOption1 == "smallest" && answerInt > newChoice)
                        answerInt = newChoice;
                    if (choices.size() == 4) break;
                }
                random_shuffle(choices.begin(), choices.end(), [](int n) { return rand() % n; });
                ...
                while (1) {
                    if (!step) {
                        ...
                        if (find(cards.begin(), cards.end(), cardCandidate) != cards.end())
                            continue;
                        ...
                        cards = {cardCandidate, cardCandidate + step, cardCandidate + step * 2,
                                 cardCandidate + step * 3};
                ...
                random_shuffle(answerOption1.begin(), answerOption1.end(),
                               [](int n) { return rand() % n; });
    ...
    string ProblemBank::checkRandom(string buf) {
        ...
        return TodoUtil::itos(
                random < int > (TodoUtil::stoi(bufVec[0]), TodoUtil::stoi(bufVec[1])));
    }
    ...
    bool ProblemBank::hasSameAnswer(string templateName, string questionOption1, string answer) {
        for (auto it : _problems) {
            if (templateName == it.templateName && questionOption1 == it.questionOption1 &&
                it.answer == answer)
                return true;
        }
        return false;
    ...
    bool ProblemBank::checkReviewFail(EggQuizTemplateStruct type) {
        ...
        if (_reviewCount == 10000) {
            NativeAlert::show("problem review failed",
                              TodoUtil::itos(type.type) + "," + type.templateName + "," +
                              type.questionOption1 + "," + type.questionOption2 + "," +
                              type.questionOption3 + "," + type.answer + "," + type.answerOption1,
                              "OK");
            _reviewCount = 0;
            return true;
        }
