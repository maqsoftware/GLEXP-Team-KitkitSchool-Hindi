...
void CurriculumManager::loadData() {
    ...
    */
    ...
    string dataSheet = "curriculumdata_" + LanguageManager::getInstance()->getCurrentLanguageCode() + ".tsv";
    string dataSheetSL = "curriculumdata_sl.tsv";
    string resourceName = LanguageManager::getInstance()->isSignLanguageMode() ? dataSheetSL
                                                                               : dataSheet;
        ...
        if (UserManager::getInstance()->isDebugMode()) {
            if (gameName == "Book") {
                ...
                if (!GameVideoScene::videoExists(gameParam)) {
                    ...
                    if (find(wrongVideos.begin(), wrongVideos.end(), gameParam) ==
                        wrongVideos.end())
                ...
                if (!CCAppController::sharedAppController()->gameExists(game.gameName)) {
                    if (find(wrongGameNames.begin(), wrongGameNames.end(), game.gameName) ==
                        wrongGameNames.end())
    ...
    auto loadDay = [&](DayCurriculum &day) {
        ...
        if (ret && entryType == "day") {
            ...
            if (day.numGames == 1 && (day.games.at(0).gameName == "EggQuizLiteracy" ||
                                      day.games.at(0).gameName == "EggQuizMath")) {
                day.isEggQuiz = true;
    ...
    auto loadLevel = [&](LevelCurriculum &level) {
        if (IS >> level.lang >> level.levelTitle >> level.category >> level.categoryLevel
               >> level.numDays) {
